...
<Update label="February 2026" tags={["New releases"]} rss={{ title: "Plan Chat Mode", description: "Conversational planning before tool execution" }}>
  ...
  - **Thinking indicator** — streamed `thinking_token` events show plan generation progress in the UI
...
<Update label="February 2026" tags={["New releases"]} rss={{ title: "Edit Chat Mode", description: "Iterative code editing with search/replace diffs" }}>

  ## Edit chat mode

  Iterative code editing without regenerating the full response:

  - **Edit mode** (`chat_mode: 'edit'`) — AI returns targeted search/replace changes merged into the original code
  - **Reference message** (`reference_message_id`) — target a specific system message for editing
  - **Toggle switch** — Chat/Edit toggle next to the send button, auto-selects last code message
  - **Edit button** — per-message "Edit" action on system messages with code blocks
  - **Language preservation** — merged code retains the original language identifier (e.g., ```html)
  - **Automatic fallback** — if changes can't be applied, falls back to full code regeneration
