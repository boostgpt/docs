...
When your agent runs in **Agent** reasoning mode, it has access to a powerful set of built-in tools. These tools let your agent search knowledge, manage files, remember users, browse the web, plan complex tasks, and more.
...
The official dashboard assistant, **Boost**, can use internal BoostGPT platform tools to inspect and configure your agents when you ask it to. These tools are user-scoped and are designed for platform operations such as listing agents, checking setup readiness, enabling workspace features, publishing agents, applying templates, and preparing test conversations.
...
| **Input** | Waits for the visitor to respond | Ask User |
