...
- `subscriber.joined` - Someone new reached the agent for the first time

<Note>
  `subscriber.joined` fires when a new contact is created — currently when someone new writes in by
  email, or when a chat creates a member that did not exist before. The id is kept as-is rather than
  renamed to `contact.created`, because existing automations reference it.
</Note>
