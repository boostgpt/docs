...
class OnboardingTracker {
  ...
  triggerHelp(step, timeOnStep) {
    ...
    boostgpt.trigger(eventName, {
      ...
      text: this.getMessageForStep(step)
    });
  }
  ...
  if (errors.length > 0) {
    ...
    if (errorCount >= 2) {
      boostgpt.trigger('onboarding_form_errors', {
        ...
        text: 'Having trouble with the form? I can help clarify what\'s needed.'
      });
    }
  }
});
...
async function testIntegration() {
  try {
    ...
    boostgpt.trigger('integration_test_failed', {
      ...
      text: `Connection test failed: ${error.message}. Want me to help troubleshoot?`
    });
  }
}
```
...
| Step | Example Message | Rationale |
|------|----------------|-----------|
| Account Setup | "Need help setting up your account?" | Friendly, conversational |
| Profile | "Having trouble with your profile?" | Simple and non-intrusive |
| Team Invite | "Want help inviting your team?" | May need explanation of roles |
| Integration | "Stuck on the integration? I can walk you through it!" | Complex, needs detailed help |
| First Project | "Need help creating your first project?" | Encouraging, feature discovery |
