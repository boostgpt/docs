...
<ParamField path="chat_mode" type="string" default="default">
  Chat mode: `default` for normal chat, `edit` for iterative code editing, `plan` for conversational planning before tool execution. Edit mode applies targeted changes to existing code instead of regenerating it. Plan mode proposes a step-by-step plan for review before executing.
</ParamField>

<ParamField path="reference_message_id" type="string">
  UUID of the system message containing code to edit. Required when `chat_mode` is `edit`.
</ParamField>
