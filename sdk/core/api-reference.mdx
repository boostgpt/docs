...
<ParamField path="chat_mode" type="string" default="ask">
  Chat mode: `ask` for normal chat, `edit` for iterative code editing, `plan` for conversational planning before tool execution. Edit mode applies targeted changes to existing code instead of regenerating it. Plan mode proposes a step-by-step plan for review before executing.
</ParamField>
