...
When an agent uses `agent` reasoning mode, it has access to BoostGPT built-in tools for knowledge, memory, files, planning, communication, automation, workspace delivery, and platform operations.

<Info>
File tools (`write`, `read`, `edit`, `grep`, `glob`, `delete`, `ls`) and the `bash` tool require the agent's **Workspace** setting to be enabled. Planning tools (`create_plan`, `update_plan`, `read_plan`, `accept_plan`) require plan mode (`chat_mode: 'plan'`), which also requires workspace.
</Info>
...
| `grep` | Files | Auto | Search file contents — regex, inverted match, multiline, context |
| `glob` | Files | Auto | Find files by path pattern |
...
| `bash` | Execution | Conditional | Run shell commands in the sandbox |
...
| `grep` | Auto | Search file contents — params: `pattern`, `file_path`, `include`, `exclude`, `files_without_match`, `invert_match`, `multiline`, `case_sensitive`, `context` |
| `glob` | Auto | Find files by path pattern — params: `pattern` |
