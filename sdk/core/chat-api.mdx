...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `chat_mode` | string | No | `'default'` or `'edit'` (default: `'default'`) |
| `reference_message_id` | string | When `chat_mode` is `'edit'` | UUID of the system message containing code to edit |

<Info>
Edit mode forces `reasoning_mode: 'standard'` (1 LLM call) regardless of the bot's configured reasoning mode. The response contains the fully merged code with the original language identifier preserved (e.g., ` ```html `). If the edit cannot be applied, it automatically falls back to full code regeneration.
</Info>
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `chat_mode` | string | No | `'default'`, `'edit'`, or `'plan'` (default: `'default'`) |
