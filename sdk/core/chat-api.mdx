...
<Note>
A tool continuation is a separate HTTP request and inherits nothing from the call that produced it.
If you set `memory`, `source_ids` or `tags` on `chat()`, send the same values to
[`executeTool()`](/sdk/core/api-reference#executetool) — otherwise the continuation falls back to the
agent's own memory setting and searches unscoped.
</Note>
...
at all, so `memory: false` means the agent cannot reach back into its knowledge mid-turn either.
